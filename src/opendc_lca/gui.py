...
from .api import analyze_scenario, audit_scenario, summarize_performance_csv
...
<button id="scenarioTab" class="active" onclick="showTab('scenario')">Scenario analysis</button>
<button id="performanceTab" onclick="showTab('performance')">Performance map</button></div>
<section id="scenario" class="panel"><div class="notice">Screening output does not by itself authorize a public comparative claim. Audit findings are shown with every result.</div>
...
<script>
let lastResult=null;
function showTab(name){for(const x of ['scenario','performance']){document.getElementById(x).classList.toggle('hidden',x!==name);document.getElementById(x+'Tab').classList.toggle('active',x===name)}}
